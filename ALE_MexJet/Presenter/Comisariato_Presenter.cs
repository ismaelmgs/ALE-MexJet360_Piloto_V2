...
{
    public class Comisariato_Presenter: BasePresenter<IViewCat>
    {
        private readonly DBComisariato oIGestCat;

        public Comisariato_Presenter(IViewCat oView, DBComisariato oGC)
            : base(oView)
        {
            oIGestCat = oGC;
        }
        ...
        protected override void SaveObj_Presenter(object sender, EventArgs e)
        {
            try
            {
                Comisariato oComisariato = oCatalogo;
                int id = oIGestCat.DBUpdate(oComisariato);
                if (id > 0)
                {
        ...
        protected override void NewObj_Presenter(object sender, EventArgs e)
        {
            ...
            {
                Comisariato oComisariato = oCatalogo;
                int id = oIGestCat.DBSave(oComisariato);
                if (id > 0)
                {
        ...
        protected override void DeleteObj_Presenter(object sender, EventArgs e)
        {
            ...
            {
                Comisariato oComisariato = oCatalogo;
                int id = oIGestCat.DBDelete(oComisariato);
                if (id > 0)
                {
        ...
        private Comisariato oCatalogo
        {
            get
            {
                Comisariato oComisariato = new Comisariato();
                switch (oIView.eCrud)
                {
                    case Enumeraciones.TipoOperacion.Insertar:
                        ASPxDataInsertingEventArgs eI = (ASPxDataInsertingEventArgs)oIView.oCrud;
                        oComisariato.iId = 0;
                        oComisariato.sDescripcion = eI.NewValues["Descripcion"].S();
                        oComisariato.iStatus = 1;

                        break;
                    case Enumeraciones.TipoOperacion.Actualizar:
                        ASPxDataUpdatingEventArgs eU = (ASPxDataUpdatingEventArgs)oIView.oCrud;

                        oComisariato.iId = eU.Keys[0].S().I();
                        oComisariato.sDescripcion = eU.NewValues["Descripcion"].S();
                        oComisariato.iStatus = eU.NewValues["Status"].S().I();

                        break;
                    case Enumeraciones.TipoOperacion.Validar:
                        ASPxDataValidationEventArgs eV = (ASPxDataValidationEventArgs)oIView.oCrud;
                        oComisariato.sDescripcion = eV.NewValues["Descripcion"].S();
                        oComisariato.iStatus = eV.NewValues["Status"].S().I();

                        break;
                    case Enumeraciones.TipoOperacion.Eliminar:
                        ASPxDataDeletingEventArgs eD = (ASPxDataDeletingEventArgs)oIView.oCrud;
                        oComisariato.iId = eD.Keys[0].S().I();
                        oComisariato.sDescripcion = eD.Values["Descripcion"].S();
                        break;
                }
